...
Style conventions for the `ha-homekit-secure-video` project. Before committing,
run `uv run ruff format --check .`, `uv run ruff check .` and
`uv run mypy custom_components/homekit_secure_video` — all must exit cleanly.
...
- User-facing strings live in `custom_components/homekit_secure_video/translations/{en,pt-BR}.json`
...
- **`type` aliases are the exception: they live in `data/__init__.py`**
  ...
  `HomeKitSecureVideoConfigEntry`), not in their own files.
...
- **One class per entity.** Every entity gets its own dedicated class — never
  ...
  - Do write `<DOMAIN><Name><Platform>` (e.g. `HomeKitSecureVideoStatusSensor`,
    `HomeKitSecureVideoCancelButton`, `HomeKitSecureVideoDoorBinarySensor`).
...
- Public classes are prefixed with `HomeKitSecureVideo` (rename to
  `<YourDomain>` when forking).
- Concrete platform entities end with the entity type:
  `HomeKitSecureVideoSensor`, `HomeKitSecureVideoBinarySensor`,
  `HomeKitSecureVideoSwitch`.
- Exception classes end with `Error`: `HomeKitSecureVideoApiClientError`,
  `…CommunicationError`, `…AuthenticationError`.
- Private attributes / functions are prefixed with `_`.

## Typing

**Strict typing. No generics, no `Any`.** Mypy (`uv run mypy custom_components/homekit_secure_video`) enforces this.
...
- `TypedDict` for known dict / JSON shapes (see the `data/` package for the
  canonical examples: `HomeKitSecureVideoPost`, `HomeKitSecureVideoConfigData`,
  `HomeKitSecureVideoOptionsData`, `HomeKitSecureVideoDiagnosticsPayload`,
  one per file).
- `@dataclass` for structured records (`HomeKitSecureVideoData` in
  `data/runtime.py`).
...
- Move type-only imports into a `TYPE_CHECKING` block (Ruff `TC001`/`TC003`):
  ...
  if TYPE_CHECKING:
      ...
      from .data import HomeKitSecureVideoConfigData
  ```
...
- Custom exceptions get the same hierarchy:
  `HomeKitSecureVideoApiClientError` (base) → `…CommunicationError` (timeout,
...
- All API state flows through `entry.runtime_data: HomeKitSecureVideoData`
  ...
  not.
- The coordinator is typed as `DataUpdateCoordinator[HomeKitSecureVideoPost]`
...
- `config_flow.py` carries `user`, `reauth`, `reauth_confirm` and `reconfigure`
  ...
  builder.
- `options_flow.py` holds the single `HomeKitSecureVideoOptionsFlow`
  class. New options keys go into the `HomeKitSecureVideoOptionsData`
...
- `repairs.py` exposes `async_create_fix_flow`. Sample helpers like
  ...
  in the integration.
- `diagnostics.py` returns `HomeKitSecureVideoDiagnosticsPayload`. Sensitive
...
- Brand assets live under `custom_components/<domain>/brand/` — `icon.png`,
  `logo.png` (+ `@2x` variants) and `icon.svg`. This repository ships the Apple
  HomeKit artwork described in
  [`brand/README.md`](./custom_components/homekit_secure_video/brand/README.md);
  the assets still have to be registered in
...
- `quality_scale.yaml` is **optional** and this repository does not ship one. It
...
- Mypy configuration lives in `pyproject.toml` (`[tool.mypy]`). Run both with
  `uv run ruff check .` and `uv run mypy custom_components/homekit_secure_video`.
- After every change run `uv run ruff format --check .`, `uv run ruff check .`,
  `uv run mypy custom_components/homekit_secure_video` and `uv run pytest`.
