...
**What HomeKit negotiated has to survive a restart.** A hub remembers the negotiation and does not repeat it on every connection: reconnecting to an accessory that came back empty, it writes `Active` = 1, reads `SelectedCameraRecordingConfiguration`, gets the failure, subscribes, and never writes a configuration again — recording stays off with nothing above DEBUG but pyhap's one-line error per read (issue #19, on iOS/tvOS 27 hubs; the hubs this was built against re-negotiate from the `Supported*` values on every connection, which is why it never showed here). `accessory/recording_state_store.py` therefore keeps the selected configuration, both `Active` flags and the three operating-mode switches in `.storage/homekit_secure_video.<entry_id>.recording`, exactly what HAP-NodeJS's `RecordingManagement.serialize` persists, and the manager restores it before publishing the accessory. The selected configuration is only taken back while the offer it was selected from is unchanged — `HomeKitSecureVideoSupportedConfiguration.fingerprint` is the digest the state carries — and the whole file goes with the pairing: "Reset pairing" and entry removal delete it. The state also carries the source profile the offer was built from, and the manager falls back to it when the camera has a stream but does not answer the probe: an unanswered probe would otherwise turn the offer into the full catalogue at 30 fps, change the fingerprint, and discard the negotiation the hub expects to find.
